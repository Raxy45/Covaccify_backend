...
function sendEmailUnsubcribe(user) {
  ...
  let mailOptions = {
    from: process.env.EMAIL,
    to: user.email,
    subject: `UNSUBSCRIBED FROM COVACCIFY SUCCESSFULLY`,
    text: messageOP,
    html: messageOP,
  };
  return new Promise((resolve, reject) => {
...
function sendWhatsappMessage(user, centers) {
  ...
  messageOP += createMessageForWhatsapp(centers);
  messageOP += `</body></html>`;
  client.messages
    .create({
...
function createMessageForWhatsapp(xD) {
  var str = "";
  xD.forEach((center, index) => {
